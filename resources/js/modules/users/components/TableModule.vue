        ...
        <div class="col-sm-12 table-responsive">
          <table class="table align-items-center table-flush dataTable no-footer" id="datatable-basic" role="grid"
            ...
            <tr role="row">
              ...
              <th scope="col" class="sorting" tabindex="0" aria-controls="datatable-basic" rowspan="1" colspan="1"
                  aria-label="Email: activate to sort column ascending">Correo
              </th>
              <th scope="col" class="sorting" tabindex="0" aria-controls="datatable-basic" rowspan="1" colspan="1"
            ...
            <tr role="row" class="odd" v-for="(item,index) in list" :key="`row_${index}`">
              ...
              <td>{{ item.roles.length > 0 ? item.roles[0].name : 'Sin rol' }}</td>
              <td class="text-right">
                <div class="dropdown">
...
export default {
  ...
  methods: {
    ...
    editItem(item){
      this.$router.push({name:'updateusers',params:{ status: 'EDIT', item: item }})
    },
    async deleteItem(item){
      ...
      if(result.isConfirmed){
        try{
          const response = await axios.delete(`/api/user/${item.id}`,{...item})
